    ...
    <main class="flex-1 ml-64 p-6 lg:p-8">
        <div class="max-w-7xl mx-auto">
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
                <div>
                    <h1 class="text-2xl font-bold" style="color:#FFFFFF">Mantenedor de Agentes</h1>
                    <p class="mt-1" style="color:rgba(255,255,255,0.7)">Asigna las sucursales que atiende cada agente de ventas</p>
                </div>
            </div>
            ...
            <div class="card rounded-2xl p-6 mb-6">
                <h2 class="font-semibold mb-4" style="color:#FFFFFF"><i class="fas fa-user-plus mr-2" style="color:#00A3E0"></i> Crear Agente de Ventas</h2>
                <form method="POST" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                    <input type="hidden" name="accion" value="crear">
                    <div>
                        <label class="block text-xs font-medium mb-1" style="color:rgba(255,255,255,0.7)">Nombre</label>
                        <input type="text" name="nombre" required placeholder="Nombre del agente"
                               class="w-full px-3 py-2.5 rounded-xl outline-none text-sm" style="border:1px solid rgba(255,255,255,0.15);background:rgba(255,255,255,0.08);color:#FFFFFF">
                    </div>
                    <div>
                        <label class="block text-xs font-medium mb-1" style="color:rgba(255,255,255,0.7)">Email</label>
                        <input type="email" name="email" required placeholder="agente@redsalud.cl"
                               class="w-full px-3 py-2.5 rounded-xl outline-none text-sm" style="border:1px solid rgba(255,255,255,0.15);background:rgba(255,255,255,0.08);color:#FFFFFF">
                    </div>
                    <div>
                        <label class="block text-xs font-medium mb-1" style="color:rgba(255,255,255,0.7)">Usuario (opcional)</label>
                        <input type="text" name="username" placeholder="auto desde email"
                               class="w-full px-3 py-2.5 rounded-xl outline-none text-sm" style="border:1px solid rgba(255,255,255,0.15);background:rgba(255,255,255,0.08);color:#FFFFFF">
                    </div>
                    <div>
                        <label class="block text-xs font-medium mb-1" style="color:rgba(255,255,255,0.7)">Contraseña</label>
                        <input type="password" name="password" required placeholder="••••••••"
                               class="w-full px-3 py-2.5 rounded-xl outline-none text-sm" style="border:1px solid rgba(255,255,255,0.15);background:rgba(255,255,255,0.08);color:#FFFFFF">
                    </div>
                    <div>
                        <label class="block text-xs font-medium mb-1" style="color:rgba(255,255,255,0.7)">Número de contacto</label>
                        <input type="text" name="numero_contacto" placeholder="+569..."
                               class="w-full px-3 py-2.5 rounded-xl outline-none text-sm" style="border:1px solid rgba(255,255,255,0.15);background:rgba(255,255,255,0.08);color:#FFFFFF">
                    </div>
                    <div>
                        <label class="block text-xs font-medium mb-1" style="color:rgba(255,255,255,0.7)">Límite de leads</label>
            ...
            <?php if (empty($agentes)): ?>
                <div class="card rounded-2xl p-12 text-center" style="color:rgba(255,255,255,0.7)">
            ...
            <?php else: ?>
                <form method="POST" id="form-asignar" class="card rounded-2xl overflow-hidden">
                    ...
                    <div class="overflow-x-auto">
                        <table class="w-full text-sm">
                            ...
                            <tbody class="divide-y" style="border-color:rgba(255,255,255,0.15)">
                                ...
                                <tr class="hover:bg-white/5 transition-colors <?= (int)$a['activo'] ? '' : 'opacity-50' ?>">
                                    <td class="px-6 py-4 align-top">
                                        <p class="font-medium" style="color:#FFFFFF">
                                            <?= htmlspecialchars($a['nombre']) ?>
                                            <?php if (!(int)$a['activo']): ?>
                                                <span class="ml-2 inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-bold text-white" style="background:rgba(255,255,255,0.5)">INACTIVO</span>
                                            <?php endif; ?>
                                        </p>
                                        <p class="text-xs mt-0.5" style="color:rgba(255,255,255,0.5)"><?= htmlspecialchars($a['email']) ?></p>
                                        <p class="text-xs mt-0.5 font-mono" style="color:rgba(255,255,255,0.5)">@<?= htmlspecialchars($a['username'] ?? $a['email']) ?></p>
                                    </td>
                                    <td class="px-6 py-4 align-top">
                                        <input type="text" form="form-agente-<?= (int)$a['id'] ?>" name="numero_contacto" value="<?= htmlspecialchars($a['numero_contacto'] ?? '') ?>"
                                               placeholder="+569..."
                                               class="w-40 px-2 py-1.5 text-xs rounded-lg outline-none" style="border:1px solid rgba(255,255,255,0.15);background:rgba(255,255,255,0.08);color:#FFFFFF">
                                    </td>
                                    <td class="px-6 py-4 align-top">
                                        <input type="number" form="form-agente-<?= (int)$a['id'] ?>" name="limite_leads" min="0" value="<?= (int)$a['limite_leads'] ?>"
                                               class="w-20 px-2 py-1.5 text-xs rounded-lg outline-none" style="border:1px solid rgba(255,255,255,0.15);background:rgba(255,255,255,0.08);color:#FFFFFF">
                                    </td>
                                    <td class="px-6 py-4">
                                        <div class="flex flex-wrap gap-3">
                                            <?php foreach ($sucursales as $s): ?>
                                                <label class="inline-flex items-center gap-2 cursor-pointer select-none px-3 py-1.5 rounded-lg border text-xs transition-colors"
                                                       style="border-color:rgba(255,255,255,0.15);background:rgba(255,255,255,0.08);color:#FFFFFF">
                                                    <input type="checkbox" name="asignacion[<?= (int)$a['id'] ?>][]" value="<?= htmlspecialchars($s) ?>"
                                                           class="w-4 h-4 rounded border-white/30 text-[#00A3E0] focus:ring-[#00A3E0]"
                                    ...
                                    <td class="px-6 py-4 text-right align-top whitespace-nowrap">
                                        <button type="submit" form="form-agente-<?= (int)$a['id'] ?>" class="px-3 py-1.5 rounded-lg text-xs font-medium transition-colors hover:bg-emerald-50" style="border:1px solid rgba(255,255,255,0.15);color:#16a34a">
                                        ...
                                        <?php if ((int)$a['activo']): ?>
                                            <button type="button" onclick="desactivarAgente(<?= (int)$a['id'] ?>, '<?= htmlspecialchars(addslashes($a['nombre']), ENT_QUOTES) ?>')"
                                                    class="ml-1 px-3 py-1.5 rounded-lg text-xs font-medium transition-colors hover:bg-red-50" style="border:1px solid rgba(255,255,255,0.15);color:#E53E3E">
                    ...
                    <div class="px-6 py-4 flex items-center justify-between" style="background:rgba(255,255,255,0.08)">
                        <p class="text-xs" style="color:rgba(255,255,255,0.7)">El agente verá los leads de todas las sucursales marcadas.</p>
                        <button type="submit" class="btn-primary px-6 py-2.5 rounded-xl text-sm font-medium text-white">
