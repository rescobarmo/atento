...
$pdo = getDB();
try {
    $pdo->exec("CREATE TABLE IF NOT EXISTS clientesredsaud (
    ...
    echo "<h2 style='color:green;font-family:sans-serif'>OK: tabla clientesredsaud creada/verificada.</h2>";
} catch (Exception $e) {
    echo "<h2 style='color:red;font-family:sans-serif'>ERROR crear tabla: " . htmlspecialchars($e->getMessage()) . "</h2>";
}
try {
    $pdo->exec("ALTER TABLE clientesredsaud ADD COLUMN nombre VARCHAR(255) NOT NULL AFTER id");
    echo "<p style='color:green'>OK: columna nombre agregada</p>";
} catch (Exception $e) {
    echo "<p style='color:green'>OK: columna nombre ya existe</p>";
}
try {
    $pdo->exec("ALTER TABLE clientesredsaud ADD COLUMN numero VARCHAR(50) NOT NULL AFTER nombre");
    echo "<p style='color:green'>OK: columna numero agregada</p>";
} catch (Exception $e) {
    echo "<p style='color:green'>OK: columna numero ya existe</p>";
}
try {
    $pdo->exec("ALTER TABLE clientesredsaud ADD COLUMN sucursal VARCHAR(255) DEFAULT NULL AFTER numero");
    echo "<p style='color:green'>OK: columna sucursal agregada</p>";
} catch (Exception $e) {
    echo "<p style='color:green'>OK: columna sucursal ya existe</p>";
}
try {
    $pdo->exec("ALTER TABLE clientesredsaud ADD COLUMN created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP");
